...
#  before_filter :backdated?, :only => [:create, :update]
  ...
  def cancel
    ...
    if course.save(validate: false)         # Don't bother with validation since the class is being cancelled
      ...
      flash_message :info, "The class <em>#{ActionController::Base.helpers.sanitize course.title}</em> was successfully cancelled.".html_safe
    else
...
#      Notification.uncancellation(course).deliver_later(:queue => 'notifications')
      flash_message :info, "The class <em>#{ActionController::Base.helpers.sanitize course.title}</em> was successfully uncancelled.".html_safe
    else
  ...
  private
    def process_params
      ...
      if params[:course].has_key?(:sections_attributes)
